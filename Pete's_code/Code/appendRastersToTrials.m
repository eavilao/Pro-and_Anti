...
reactionTimeLimit = [0 0.5]; %how many seconds after the go cue [atLeast atMost] to count as valid reaction times
...
for trialNum = 1:numTrials
    ...
    if ~isempty(thisTrialStart) && ~isempty(thisTrialEnd) && ~isempty(goCueTime)
      ...
      newTrialStructure(trialNum).eyeAngle = NaN(1,eyeSegmentLength);
       ...
       %reactionTimeLimit
       thisTrialValidSaccadeNumbers = thisTrialSaccadeNumbers(allSaccStarts(thisTrialSaccadeNumbers)>goCueTime+reactionTimeLimit(1) & allSaccStarts(thisTrialSaccadeNumbers)<goCueTime+reactionTimeLimit(2));
        ...
        if isempty(thisTrialValidSaccadeNumbers)
            ...
            if saccadeTimeIndex<=eyeSegmentLength
                if  max(abs(newTrialStructure(trialNum).eyePositionX(saccadeTimeIndex-100:saccadeTimeIndex-20)))>1 || max(abs(newTrialStructure(trialNum).eyePositionY(saccadeTimeIndex-100:saccadeTimeIndex-20)))>1
                    newTrialStructure(trialNum).goCueTime = relativeGoCueTime;
                    %if no valid saccades then everything should be Nan
                    newTrialStructure(trialNum).saccadeTime = NaN;
                    newTrialStructure(trialNum).reactionTime = NaN;
