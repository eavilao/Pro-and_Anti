function [f, ca, o] = createSpider(data,tle,rng,lbl,leg,f,varargin)
...
%
% %PH% varargin(1) is std's of data in same format to allow confidence limits 
%
...
% editing by peter holland (2013), marked %ph%
% enables use of a varargin containing standard deviations (same size as
% data matrix). These are then used to plot confidence limits and patches
% around them.
% Also tick labels removed from half of the axes for clarity
%
%
...
if nargin>6
    %do the same for confidence limits
    %for now only work in the case of two variables
    %conLims = varargin{1}(:,1); 
     conLims = [data(:,1)-varargin{1}(:,1) data(:,1)+varargin{1}(:,1) data(:,2)-varargin{1}(:,2) data(:,2)+varargin{1}(:,2)];
     c2 = size(conLims,2)
     
     magCon = (conLims - rng(:,1) * ones(1,c2)) ./ (diff(rng,[],2) * ones(1,c2));
    % scale trimming
    magCon(mag < 0) = 0; magCon(magCon > 1) = 1;
    % wrap data (close the last axis to the first)
    ang = angw(1:end-1); angwv = angw * ones(1,c2); magwCon = [magCon; magCon(1,:)];
...
for ii = 1:2:7 %ph% chaned from 'r'
	...
	for jj = 1:4
       labelVal = chop(rng(ii,1) + inc(jj)*diff(rng(ii,:)),2);
		temp = text([cos(ang(ii)) * inc(jj) + sin(ang(ii)) * tx(jj)], ...
				[sin(ang(ii)) * inc(jj) - cos(ang(ii)) * tx(jj)], ...
				num2str(round(labelVal*100)/100), ...  %ph changed to force to 3 digits to stop overlap
				'fontsize',8);
...
if nargin>6
    o2 = polar(ca,angw*ones(1,c2),magwCon); %'color',[0 1 0;0 1 0;1 0 0;1 0 0]
    x1 = get(o2,'Xdata')
    y1 = get(o2,'Ydata')
    patch([x1{1} fliplr(x1{2})],[y1{1} fliplr(y1{2})],'g','facealpha',0.5)
    patch([x1{3} fliplr(x1{4})],[y1{3} fliplr(y1{4})],'r','facealpha',0.5)
    o = [o; o2];
end
