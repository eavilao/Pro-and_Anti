...
smallTimeWindow = [0.3 0.5]; %time window for actual rasters
 ...
 if exist([resultsDir filesep wholeNeuronNameEnd],'file')~=2 %delete the file ans start again every time
    ...
    wholeNeuronResults = struct('allStableTrials',[],'selectedTrials',[]); %allStableTrails contaiins the trial structure for all trials in a neuron within the periods marked as stabel by Eric.
