...
for cellNum = 1:length(wholeNeuronResults);
    for trialNum = 1:length(wholeNeuronResults(cellNum).allStableTrials);
        if ~isempty(wholeNeuronResults(cellNum).allStableTrials(trialNum).alignedSpikes)
            ...
            %neural data      
            spks =  wholeNeuronResults(cellNum).allStableTrials(trialNum).alignedSpikes{2}; % extract spikes -  contains spike times for CS aligned to trial onset
            ...
            trialData(cellNum).trial.neural(trialNum).tspk_CS = spks(indx_trial_spks);
            trialData(cellNum).trial.neural(trialNum).tspk_CS_align_sacc =  trialData(cellNum).trial.neural(trialNum).tspk_SS-trialData(cellNum).trial.behav(trialNum).saccadeOnset; % contains spike times for CS aligned to sacc onset
            else
                trialData(cellNum).trial.neural(trialNum).tspk_CS = []; 
                trialData(cellNum).trial.neural(trialNum).tspk_CS_align_sacc = [];
            end
            trialData(cellNum).id = 'CS';
...
for cellNum = 1:length(trialData)
    ...
    [trialData(cellNum).anti.neural.rate_pst,trialData(cellNum).anti.neural.ts] = Spiketimes2Rate(trialData(cellNum).anti.neural.trial,timepoints,binwidth,analyse_sacc_win,id); % aligned to trial onset
...
for cellNum = 1:length(trialData)
    ...
    %% Stats per neuron   CHECK - TODO (right now is the same as in SS)
